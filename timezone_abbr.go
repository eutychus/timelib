...
// TimezoneAbbreviation represents a timezone abbreviation mapping
...
// TimezoneAbbreviationTable is the complete timezone abbreviation lookup table
// Ported from timelib timezonemap.h
var TimezoneAbbreviationTable = []TimezoneAbbreviation{
	...
	// Military timezone abbreviations (single letters A-Z)
	// These have empty TzID (NULL in C) and represent generic UTC offsets
	{"a", 0, 3600, ""},
	{"b", 0, 7200, ""},
	{"c", 0, 10800, ""},
	{"d", 0, 14400, ""},
...
// LookupTimezoneAbbr looks up a timezone abbreviation and returns the timezone ID
// This matches the C function abbr_search() in parse_date.c
func LookupTimezoneAbbr(abbr string, gmtOffset int32, isDST int) *TimezoneAbbreviation {
	// Special case for UTC/GMT/Z (case-insensitive)
	abbrLower := strings.ToLower(abbr)
	if abbrLower == "utc" || abbrLower == "gmt" || abbrLower == "z" {
	...
	var firstFound *TimezoneAbbreviation

	// Search for exact match
	for i := range TimezoneAbbreviationTable {
		entry := &TimezoneAbbreviationTable[i]
		if strings.EqualFold(entry.Abbr, abbr) {
			if firstFound == nil {
				firstFound = entry
				// If no offset specified (-1), return first match
				if gmtOffset == -1 {
					return entry
				}
			}
			// Check for offset match
			if entry.OffsetSec == gmtOffset {
				return entry
			}
		}
	}
